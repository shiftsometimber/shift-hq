...
const report={proof:'G3_006_HQ_LIVE_PRESENTATION_V3_STYLESHEET_SETTLED',assets:{},cases:[],failures:[]};
...
for(const [asset,markers] of Object.entries({
  ...
  'hq.js':['installKnowledgeEditorialPresentation','Useful first. Reviewed before it goes live.','Editorial review','data-knowledge-publish','/v1/hq/articles/${id}/review','hq-knowledge-editorial.css?v=2'],
...
try{
  for(const [id,viewport] of Object.entries({desktop:{width:1440,height:900},mobile390:{width:390,height:844}})){
    ...
    try{
      ...
      await page.waitForFunction(()=>{const link=document.querySelector('link[data-g3-006]');return !!(link&&link.sheet)},null,{timeout:10000});
      if(viewport.width<=900){await page.waitForFunction(()=>{const s=document.querySelector('#knowledgehub'),w=s?.querySelector('.tablewrap');if(!s||!w)return false;const sw=s.getBoundingClientRect().width,ww=w.getBoundingClientRect().width;return sw<=document.documentElement.clientWidth+1&&ww<=document.documentElement.clientWidth+1},null,{timeout:10000});}
      row.rendered=await page.evaluate(()=>{const b=document.querySelector('button[data-view="knowledgehub"]'),s=document.querySelector('#knowledgehub'),intro=document.querySelector('.knowledge-editorial-intro'),wrap=s.querySelector('.tablewrap'),head=[...s.querySelectorAll('thead th')].map(x=>x.textContent.trim()),link=document.querySelector('link[data-g3-006]');b.click();const r=s.getBoundingClientRect(),wr=wrap?.getBoundingClientRect(),ws=wrap?getComputedStyle(wrap):null;return{nav:b.textContent.trim(),sectionId:s.id,intro:intro.innerText.replace(/\s+/g,' ').trim(),headings:head,active:s.classList.contains('active'),stylesheet:link?.href||'',stylesheetReady:!!link?.sheet,rootOverflow:document.documentElement.scrollWidth-document.documentElement.clientWidth,section:{w:Math.round(r.width),left:Math.round(r.left),right:Math.round(r.right)},tableWrap:wrap?{w:Math.round(wr.width),scrollWidth:wrap.scrollWidth,overflowX:ws.overflowX}:null};});
      if(!/Knowledge Hub CMS/.test(row.rendered.nav))fail(`${id}-nav`,row.rendered.nav);if(!/Useful first\. Reviewed before it goes live\./.test(row.rendered.intro))fail(`${id}-intro`,row.rendered.intro);if(!row.rendered.headings.includes('Editorial review'))fail(`${id}-review-column`,JSON.stringify(row.rendered.headings));if(!row.rendered.stylesheetReady||!/hq-knowledge-editorial\.css\?v=2/.test(row.rendered.stylesheet))fail(`${id}-stylesheet`,row.rendered.stylesheet||'not loaded');if(row.rendered.rootOverflow>0)fail(`${id}-overflow`,`${row.rendered.rootOverflow}px`);if(!row.rendered.active)fail(`${id}-knowledge-view`,'knowledge section did not activate');if(id==='mobile390'&&(!row.rendered.tableWrap||row.rendered.tableWrap.w>390||!['auto','scroll'].includes(row.rendered.tableWrap.overflowX)))fail(`${id}-table-containment`,JSON.stringify(row.rendered.tableWrap));if(row.pageErrors.length)fail(`${id}-page-errors`,JSON.stringify(row.pageErrors));
