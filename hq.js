  ...
  legacy.onload = () => {
    ...
    async function loadRadar(){
      try {
        ...
        $('#radarQueueCount').textContent=events.length; $('#radarVerifiedCount').textContent=events.filter(x=>x.verification?.verified&&!x.source_change).length; $('#radarMedicineCount').textContent=meds.length; $('#radarForwardCount').textContent=forward.length;
        $('#radarRows').innerHTML=events.length?events.map(x=>`<tr data-radar-event="${Number(x.id)}"><td><b>${esc(x.headline)}</b><p class="radar-summary">${radarSummary(x)}</p><small>${esc(x.regulator||x.event_type||'development')}</small></td><td>${esc(x.region||'GLOBAL')}</td><td>${badge(x.status)}${radarSourceWarning(x,true)}</td><td>${x.source_change?'<span class="badge">Fresh review required</span>':x.verification?.verified?'<span class="badge">Verified</span>':'<span class="badge">Needs evidence</span>'}<br><small>${esc(x.verification?.reason||'')}</small></td><td>Urgency ${Number(x.urgency_score||0)}<br>Relevance ${Number(x.relevance_score||0)}</td><td>${radarActions(x)}</td></tr>`).join(''):'<tr><td colspan="6">Nothing waiting for review.</td></tr>';
    ...
    function radarCanReview(){return ['owner','admin','operations','editor'].includes(S.me?.role)}
    function radarCanPublish(){return ['owner','admin'].includes(S.me?.role)}
    function radarSourceUrl(value){try{const url=new URL(String(value||''));return url.protocol==='https:'&&!url.username&&!url.password?url.href:null}catch{return null}}
    function radarObservedAt(value){
      const raw=String(value||''),utc=/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}(?:\.\d+)?$/.test(raw)?raw.replace(' ','T')+'Z':raw,date=new Date(utc);
      return Number.isNaN(date.getTime())?'Not recorded':new Intl.DateTimeFormat('en-GB',{dateStyle:'medium',timeStyle:'short',timeZone:'Europe/London'}).format(date)+' UK time';
    }
    function radarSourceWarning(event,compact=false){
      const change=event.source_change;if(!change)return '';
      if(compact)return '<p class="radar-source-change-label">Source changed — fresh review required</p>';
      const observed=change.observation||{},url=radarSourceUrl(change.url),evidence=Array.isArray(observed.evidence)?observed.evidence:[];
      return `<section class="panel radar-source-change" aria-label="Source change requiring review"><h3>Source changed — fresh review required</h3><p>The existing article and its evidence remain below. This observation has not been approved. Saving, approving and publishing are blocked until a reviewer starts a correction.</p><p><strong>Observed:</strong> ${esc(radarObservedAt(change.observed_at))}<br><strong>Source:</strong> ${esc(change.source||'Not recorded')}</p>${url?`<p><a href="${esc(url)}" target="_blank" rel="noopener noreferrer">Open changed source</a></p>`:'<p>No valid HTTPS source link is available.</p>'}<details><summary>View changed source observation</summary><p>${esc(observed.headline||'No headline supplied')}</p><p>${esc(observed.verification?.reason||'Evidence requires review.')}</p>${evidence.map(source=>{const link=radarSourceUrl(source.url||source.source_url);return `<p><strong>${esc(source.authority||'Source')}</strong><br>${esc(source.title||'No title supplied')}<br><small>Source date: ${esc(source.source_date||'Not supplied')}</small>${link?`<br><a href="${esc(link)}" target="_blank" rel="noopener noreferrer">Read observed evidence</a>`:''}</p>`}).join('')}</details></section>`;
    }
    async function startRadarSourceCorrection(event,form,state,button){
      if(!event.source_change||!radarCanReview())return;
      const published=event.status==='published',question='Start a fresh review from this changed source? This replaces the working article package and requires fresh preparation and approval.'+(published?' The current article will stop being publicly published when the correction starts.':'')+' The previous version remains in the audit history.';
      if(!confirm(question))return;
      button.disabled=true;state.textContent='Starting source correction…';
      try{
        const result=await radarCall(`/v1/hq/radar/events/${event.id}/correct`,{method:'POST',body:JSON.stringify({note:form.elements.review_note.value})});
        if(result.ok!==true||!['verified','needs_more_evidence'].includes(result.status))throw Error(result.message||'The correction was not confirmed. Refresh the queue before continuing.');
        await loadRadar();await openRadarEvent(event.id);
        const current=$('#radarEventState');if(current)current.textContent=result.message||'Correction started. A new article package and approval are required.';
      }catch(e){state.textContent=`Correction failed: ${e.message}`;button.disabled=false}
    }

    function radarSummary(event){
      const content=event.content_package||{};
      const summary=[content.standfirst,content.what_changed,content.seo?.description]
        .find(value=>typeof value==='string'&&value.trim());
      return esc(summary?summary.trim():'Summary not supplied — open the evidence before reviewing.');
    }
    ...
    async function openRadarEvent(id){
      ...
      if(changed)form.querySelectorAll('textarea:not([name=review_note]),input').forEach(field=>{if(field.type==='checkbox')field.disabled=true;else field.readOnly=true});
      dialog.querySelector('[data-radar-correct]')?.addEventListener('click',eventClick=>startRadarSourceCorrection(event,form,state,eventClick.currentTarget));
      dialog.querySelector('[data-radar-prepare]')?.addEventListener('click',async eventClick=>{const button=eventClick.currentTarget;button.disabled=true;state.textContent='Preparing an evidence-bound draft…';try{const result=await radarCall(`/v1/hq/radar/events/${event.id}/process`,{method:'POST',body:'{}'});if(!result.ok)throw Error(result.message||result.error||'More evidence is required before preparing a draft.');await loadRadar();await openRadarEvent(event.id);$('#radarEventState').textContent='Draft prepared. Review it before approval.'}catch(e){state.textContent=`Preparation stopped: ${e.message}`;button.disabled=false}});
      dialog.querySelector('[data-radar-save]').onclick=async()=>{if(changed||!canReview)return;state.textContent='Saving…';try{await radarCall(`/v1/hq/radar/events/${event.id}`,{method:'PATCH',body:JSON.stringify(radarPackageFrom(form))});state.textContent='Modifications saved and audited.';await loadRadar()}catch(e){state.textContent=`Save failed: ${e.message}`}};
      dialog.querySelectorAll('[data-radar-decide]').forEach(button=>button.onclick=async()=>{const action=button.dataset.radarDecide;if(!canReview||(action==='approve'&&(changed||!radarCanPublish())))return;const payload=action==='approve'?radarPackageFrom(form):{note:form.elements.review_note.value};state.textContent=`${action==='approve'?'Approving':action==='hold'?'Holding':'Declining'}…`;try{await radarCall(`/v1/hq/radar/events/${event.id}/${action}`,{method:'POST',body:JSON.stringify(payload)});state.textContent=`${action==='approve'?'Approved for publication':'Decision saved'} and audited.`;await loadRadar();setTimeout(()=>openRadarEvent(event.id),0)}catch(e){state.textContent=`Decision failed: ${e.message}`}});
    ...
    async function radarReview(id,action){
      if(action==='publish'&&radarEvents.find(event=>String(event.id)===String(id))?.source_change)return;
