...
(() => {
  ...
  legacy.onload = () => {
    ...
    const fmt = d => { if (!d) return 'â'; try { return new Date(d).toLocaleString('en-GB',{dateStyle:'medium',timeStyle:'short'}); } catch { return d; } };
    const badge = v => `<span class="badge">${esc(String(v || 'â').replaceAll('_',' '))}</span>`;
    ...
    function rationaliseKnowledgeNavigation() {
      ...
      if (knowledgeButtons[0] && knowledgeSections[0]) {
        ...
        knowledgeButtons[0].textContent='â¥ Knowledge Hub CMS';
      ...
      if (knowledgeButtons[1]) knowledgeButtons[1].textContent='â¬ Shift Brain Knowledge';
    }
    ...
    function installKnowledgeEditorialPresentation(){
      ...
      if(toolbar){
        ...
        if(!section.querySelector('.knowledge-editorial-intro')){
          toolbar.insertAdjacentHTML('afterend',`<div class="knowledge-editorial-intro"><div><p class="eyebrow">EDITORIAL STANDARD</p><h3>Useful first. Reviewed before it goes live.</h3><p>Every Knowledge Hub article keeps its editorial state, reviewer identity, review time and note. Publishing is deliberately unavailable until that approval is retained.</p></div><div class="knowledge-editorial-proof"><strong>Draft â Review â Publish</strong><span>No anonymous approval. No disappearing review trail. No bypassing the existing CMS.</span></div></div>`);
        }
      }
    ...
    function reviewCell(a){
      ...
      return `<div class="editorial-review"><strong>${esc(r.decision==='approved'?'Approved':'Changes requested')} Â· ${esc(r.reviewer_name||'Named reviewer')}</strong><small>${esc(fmt(r.reviewed_at))}${r.notes?' Â· '+esc(r.notes):''}</small></div>`;
    }
    ...
    async function loadHubArticles(){
      ...
      try {
        ...
        rows.innerHTML=articles.length?articles.map(a=>`<tr data-editorial-state="${esc(a.review?.decision||'unreviewed')}"><td><b>${esc(a.title)}</b><br><small>/${esc(a.slug)}</small></td><td>${badge(a.status)}</td><td>${esc(a.category||'â')}</td><td>${esc(a.author||'â')}</td><td>${reviewCell(a)}</td><td>${knowledgeActions(a)}</td><td>${esc(fmt(a.updated_at))}</td></tr>`).join(''):'<tr><td colspan="7">No Knowledge Hub articles yet.</td></tr>';
    ...
    function installRadarView() {
      ...
      button.dataset.view='radar'; button.textContent='â¦ Medicines Intelligence';
      ...
      section.innerHTML=`
        <div class="toolbar"><div><p class="eyebrow">SHIFT AI Â· OPERATIONS DESK</p><h2>Medicines Intelligence Desk</h2><p class="sub">Detect â verify â package â human review â publish â freshness, inside the existing HQ operating system.</p><p id="radarScanStatus" role="status"></p></div><div class="actions"><button id="scanRadar">Run source scan</button><button id="previewRadarBulk">Preview bulk approval</button><button id="previewRadarPublish">Preview approved publication</button><button id="refreshRadar">Refresh Radar</button></div></div>
        <div class="cards compact-cards"><article><span>Review queue</span><strong id="radarQueueCount">â</strong><em>needs human action</em></article><article><span>Verified</span><strong id="radarVerifiedCount">â</strong><em>evidence gate passed</em></article><article><span>Medicines</span><strong id="radarMedicineCount">â</strong><em>living registry</em></article><article><span>Forward Radar</span><strong id="radarForwardCount">â</strong><em>future milestones</em></article></div>
      ...
      $('#previewRadarBulk').onclick=async()=>{const status=$('#radarScanStatus'),button=$('#previewRadarBulk');button.disabled=true;status.textContent='Calculating verified, SEO-complete, non-duplicate articlesâ¦';try{const preview=await radarCall('/v1/hq/radar/bulk-approve-preview');window.radarBulkPreview=preview;status.innerHTML=`${Number(preview.eligible_count)} eligible for approval Â· ${Number(preview.excluded_count)} excluded Â· ${Object.entries(preview.exclusion_reasons||{}).map(([reason,count])=>`${esc(reason)}: ${Number(count)}`).join(' Â· ')} Â· destinations: ${(preview.destinations||[]).map(esc).join(', ')}. <button id="executeRadarBulk" type="button">Approve exactly ${Number(preview.eligible_count)}</button>`;$('#executeRadarBulk').onclick=async()=>{const action=$('#executeRadarBulk');action.disabled=true;status.textContent=`Approving exactly ${Number(preview.eligible_count)} reviewed articlesâ¦`;try{const result=await radarCall('/v1/hq/radar/bulk-approve',{method:'POST',body:JSON.stringify({preview_token:preview.preview_token})});status.textContent=`Bulk approval complete Â· ${Number(result.approved_count)} approved Â· ${Number(result.failed_count)} failed Â· nothing published.`;await loadRadar()}catch(e){status.textContent=`Bulk approval failed Â· ${e.message}`}}}catch(e){status.textContent=`Preview failed Â· ${e.message}`}finally{button.disabled=false}};
      $('#previewRadarPublish').onclick=()=>{const status=$('#radarScanStatus'),targets=radarEvents.filter(x=>['approved','publish_failed'].includes(x.status));window.radarPublishTargets=targets.map(x=>Number(x.id));status.innerHTML=`${targets.length} approved articles are ready for live publication. <button id="executeRadarPublish" type="button">Publish exactly ${targets.length} live</button>`;$('#executeRadarPublish').onclick=async()=>{const button=$('#executeRadarPublish');button.disabled=true;let published=0,failed=[];for(const id of window.radarPublishTargets){status.textContent=`Publishing ${published+failed.length+1} of ${window.radarPublishTargets.length}â¦`;try{await radarCall(`/v1/hq/radar/events/${id}/publish`,{method:'POST',body:'{}'});published++}catch(e){failed.push({id,error:e.message})}}status.textContent=`Publication run complete Â· ${published} published Â· ${failed.length} failed.`;window.radarPublishResult={published,failed};await loadRadar()}};
      $('#scanRadar').onclick=async()=>{const status=$('#radarScanStatus'),button=$('#scanRadar');button.disabled=true;status.textContent='Fetching authoritative medicine sourcesâ¦';try{const result=await radarCall('/v1/hq/radar/scan',{method:'POST',body:'{}'});status.textContent=`Scan complete Â· ${Number(result.detected||result.ingested||0)} developments detected.`;await loadRadar()}catch(e){status.textContent=`Scan FAILED Â· ${e.message}`}finally{button.disabled=false}};
    }
    ...
    async function loadRadar(){
      try {
        ...
        $('#radarRows').innerHTML=events.length?events.map(x=>`<tr data-radar-event="${Number(x.id)}"><td><b>${esc(x.headline)}</b><br><small>${esc(x.regulator||x.event_type||'development')}</small>${radarSeoSummary(x)}</td><td>${esc(x.region||'GLOBAL')}</td><td>${badge(x.status)}</td><td>${x.verification?.verified?'<span class="badge">Verified</span>':'<span class="badge">Needs evidence</span>'}<br><small>${esc(x.verification?.reason||'')}</small></td><td>Urgency ${Number(x.urgency_score||0)}<br>Relevance ${Number(x.relevance_score||0)}</td><td>${radarActions(x)}</td></tr>`).join(''):'<tr><td colspan="6">Nothing waiting for review.</td></tr>';
        $('#radarMedicines').innerHTML=meds.length?meds.slice(0,40).map(x=>`<div class="cms-item"><b>${esc(x.brand||x.generic_name||x.id)}</b><span>${esc(x.generic_name||'')} Â· Radar ${Number(x.radar_score||0)}</span><span>${esc(x.global_stage||'Unknown stage')} Â· UK: ${esc(x.uk_regulatory_status||'Unknown')} Â· NICE: ${esc(x.nice_status||'Unknown')} Â· NHS: ${esc(x.nhs_status||'Unknown')}</span></div>`).join(''):'<div class="empty">No approved medicines yet.</div>';
        $('#radarForward').innerHTML=forward.length?forward.slice(0,30).map(x=>`<div class="cms-item"><b>${esc(x.title||x.milestone||'Forward milestone')}</b><span>${esc(x.due_at||x.expected_at||'Date TBC')} Â· ${esc(x.status||'watching')}</span></div>`).join(''):'<div class="empty">No Forward Radar milestones yet.</div>';
        $('#radarPublicationJobs').innerHTML=jobs.length?jobs.slice(0,30).map(x=>`<div class="cms-item"><b>Event #${x.event_id} Â· ${esc(x.status)}</b><span>${esc(fmt(x.created_at))}${x.error_text?' Â· '+esc(x.error_text):''}</span>${x.status==='queued'?`<div class="row-actions"><button data-radar-publish="${x.event_id}">Publish to configured adapters</button></div>`:''}</div>`).join(''):'<div class="empty">No publication jobs yet.</div>';
    ...
    function installMedicineControls(){
      ...
      if(!button){
        ...
        button.textContent='â Medicines';
      ...
      button.onclick=()=>{
        ...
        const title=$('#title');if(title)title.textContent='Medicines, prices, stock and margin â in one place.';
    ...
    async function openMedicineControls(){
      ...
      dialog.innerHTML='<div style="padding:24px"><button type="button" data-close-medicine style="float:right" aria-label="Close">Ã</button><p class="eyebrow">MEDICINE COMMERCE</p><h2>Medicines, prices and real stock</h2><p class="sub">A treatment is buyable only when its product and strength are available and remaining stock is above zero.</p><p id="medicineControlStatus">Loading live catalogueâ¦</p><div id="medicineControlList"></div></div>';
    ...
    async function loadMedicineControls(){
      ...
      try{
        ...
        $$('[data-variant-form]').forEach(form=>form.onsubmit=async event=>{event.preventDefault();const fields=new FormData(form),message=form.querySelector('[role=status]'),button=form.querySelector('button');button.disabled=true;message.textContent='Savingâ¦';try{const result=await radarCall(`/v1/hq/medicine-variants/${form.dataset.variantForm}`,{method:'PATCH',body:JSON.stringify({costPence:Math.round(Number(fields.get('cost'))*100),sellingPricePence:Math.round(Number(fields.get('selling'))*100),stockOnHand:Number(fields.get('stock')),status:fields.get('status')})});message.textContent=`Saved Â· margin ${Number(result.marginPercent||0).toFixed(2)}%`;await loadMedicineControls()}catch(e){message.textContent=e.message}finally{button.disabled=false}});
    ...
    function radarSeoSummary(x){
      const c=x.content_package||{},seo=c.seo||{};
      const description=seo.description||c.standfirst||'SEO description missing â complete before approval.';
      const slug=seo.slug?`/${String(seo.slug).replace(/^\/+/, '')}`:'Slug missing';
      return `<div class="radar-seo-summary"><strong>SEO summary</strong><br><small>${esc(description)}</small><br><small>${esc(slug)}</small></div>`;
    }
    ...
    function radarPackageFrom(form){
      ...
      let keywords=[];try{keywords=JSON.parse(form.elements.seo_keywords?.value||'[]')}catch{throw new Error('SEO keywords must be valid JSON.')}
      contentPackage.seo={title:form.elements.seo_title?.value||'',description:form.elements.seo_description?.value||'',slug:form.elements.seo_slug?.value||'',canonical:form.elements.seo_canonical?.value||'',image:form.elements.seo_image?.value||'',image_alt:form.elements.seo_image_alt?.value||'',keywords};
    ...
    function installEvidenceDeskView(){
      ...
      const button=document.createElement('button');button.dataset.view='evidencedesk';button.textContent='â Evidence Desk';
      ...
      section.innerHTML=`
        <div class="toolbar evidence-toolbar"><div><p class="eyebrow">SHIFT EVIDENCE INBOX Â· READ ONLY</p><h2>Keep the website true when the evidence moves.</h2><p class="sub">Source â structured fact â Shift claim â exact page â recorded package. This screen reports the machine truth; it cannot grant a review or publish.</p></div><div class="toolbar-actions"><button id="evidenceRefresh">Refresh</button></div></div>
        ...
        <div id="evidenceControl" class="evidence-control">Loading control stateâ¦</div>
        ...
        <div class="cards compact-cards evidence-cards"><article><span>Allowlisted sources</span><strong id="evidenceSourceCount">â</strong><em id="evidenceSourceActive">â active</em></article><article><span>Mapped claims</span><strong id="evidenceClaimCount">â</strong><em>claim-to-page spine</em></article><article><span>Open changes</span><strong id="evidenceEventCount">â</strong><em>need mapping or a decision</em></article><article><span>Decision packages</span><strong id="evidencePackageCount">â</strong><em id="evidenceNoPublish">â no-publish decisions</em></article></div>
        <div class="panel evidence-principle"><div><p class="eyebrow">OPERATING STANDARD</p><h3>One worthwhile action. Publishing nothing is allowed.</h3><p>âEditorial acceptedâ appears only after a retained editorial acceptance. Specialist review and publication authority are independent gates; neither is inferred from a draft, a model or a staging preview.</p></div><div class="evidence-lock-stack"><span>Specialist review: package-specific</span><span>Publication: disabled unless every gate passes</span><span>Newsletter: separate approval</span><span>Social: separate approval</span></div></div>
    ...
    function evidenceEditorial(pkg){
      ...
      if(pkg.editorial_reviewed_at)return '<span class="evidence-gate">Editorial reviewed â acceptance not recorded</span>';
    ...
    async function loadEvidenceDesk(){
      try{
        ...
        $('#evidenceControl').innerHTML=`<strong>${operation.monitoring?'Controlled monitoring enabled':'Evidence Desk sealed'}</strong><span>${operation.monitoring?'Structured source checks are active':'Source checks are off'} Â· ${operation.drafting?'evidence-bound drafting enabled':'automatic drafting disabled'} Â· publication remains approval-driven</span>${operation.shutdown?`<small>Stopped: ${esc(operation.shutdown)}</small>`:''}`;
        ...
        $('#evidenceSources').innerHTML=(sources.sources||[]).map(source=>`<div class="cms-item"><b>${esc(source.name)}</b><span>${esc(source.family.toUpperCase())} Â· ${esc(source.extraction_method.replaceAll('_',' '))} Â· ${esc(source.status)}</span><span>${esc(source.authority_name)} Â· ${esc(source.canonical_url)}</span></div>`).join('')||'<div class="empty">Load the founding source registry. Sources remain draft until their adapter is proved.</div>';
        $('#evidenceClaims').innerHTML=(claims.claims||[]).map(claim=>`<div class="cms-item"><b>${esc(claim.claim_text)}</b><span>${esc(claim.risk_lane)} Â· ${esc(claim.communication_class.replaceAll('_',' '))}</span><span>${Number(claim.dependency_count)} source dependencies Â· ${Number(claim.page_count)} page placements</span></div>`).join('')||'<div class="empty">No claim-to-page dependencies have been commissioned yet.</div>';
        const list=packages.packages||[];$('#evidencePackageRows').innerHTML=list.length?list.map(pkg=>`<tr><td><b>#${Number(pkg.id)} Â· ${esc(pkg.title)}</b><br><small>${esc(pkg.summary)}</small><br>${badge(pkg.status)}</td><td><span class="evidence-lane ${esc(pkg.risk_lane)}">${esc(pkg.risk_lane)}</span><br><small>${esc(String(pkg.communication_class||'').replaceAll('_',' '))}</small></td><td>${evidenceMapping(pkg)}</td><td>${evidenceEditorial(pkg)}</td><td>${evidenceSpecialists(pkg)}</td><td>${evidencePublication(pkg,operation)}<br><span class="evidence-read-only">Read only</span></td></tr>`).join(''):'<tr><td colspan="6">Nothing is waiting in the Inbox. Silence is a valid result.</td></tr>';
    ...
    function installWebsiteUpdater(){
      ...
      const render=async()=>{
        host.innerHTML='<div class="empty">Loading controlled public fieldsâ¦</div>';
        try{
          const data=await radarCall('/v1/hq/site-content');
          const rows=data.content||[];
          host.innerHTML=rows.length?rows.map(x=>`<article class="cms-item" data-site-content="${Number(x.id)}"><b>${esc(x.label)}</b><span>${esc(x.page_path)} Â· ${badge(x.status)} Â· version ${Number(x.version||1)}</span><textarea aria-label="${esc(x.label)} text">${esc(x.draft_text||'')}</textarea><div class="actions"><button data-site-preview="${Number(x.id)}">Preview</button><button data-site-publish="${Number(x.id)}">Publish</button><button data-site-rollback="${Number(x.id)}">Rollback</button></div><p role="status"></p></article>`).join(''):'<div class="empty">No controlled public fields yet.</div>';
          $$('[data-site-preview],[data-site-publish],[data-site-rollback]').forEach(button=>button.onclick=async()=>{
            ...
            button.disabled=true;status.textContent=action==='preview'?'Preparing previewâ¦':action==='publish'?'Publishingâ¦':'Rolling backâ¦';
            try{const result=await radarCall(`/v1/hq/site-content/${id}`,{method:'PATCH',body:JSON.stringify({action,text})});if(action==='preview'){const p=result.preview||{};status.textContent=`Preview only Â· ${p.pagePath||''} Â· current: ${p.currentText||'underlying page wording'} Â· proposed: ${p.draftText||text}`;}else{status.textContent=`Committed Â· ${result.status} Â· version ${Number(result.version||0)}`;await render();}}catch(e){status.textContent=`FAILED Â· ${e.message}`;}finally{button.disabled=false;}
          });
