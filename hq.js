  ...
  legacy.onload = () => {
    ...
    async function loadRadar(){
      try {
        ...
        $('#radarRows').innerHTML=events.length?events.map(x=>`<tr data-radar-event="${Number(x.id)}"><td><b>${esc(x.headline)}</b><p class="radar-summary">${radarSummary(x)}</p><small>${esc(x.regulator||x.event_type||'development')}</small></td><td>${esc(x.region||'GLOBAL')}</td><td>${badge(x.status)}</td><td>${x.verification?.verified?'<span class="badge">Verified</span>':'<span class="badge">Needs evidence</span>'}<br><small>${esc(x.verification?.reason||'')}</small></td><td>Urgency ${Number(x.urgency_score||0)}<br>Relevance ${Number(x.relevance_score||0)}</td><td>${radarActions(x)}</td></tr>`).join(''):'<tr><td colspan="6">Nothing waiting for review.</td></tr>';
